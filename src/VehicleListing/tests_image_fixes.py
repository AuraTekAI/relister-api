...
class ExtensionPayloadGumtreeGuardTests(TestCase):
    ...
    regardless of hosting readiness; it's covered on its own below and
    explicitly disabled in the other tests so they exercise the real,
    non-bypassed behaviour."""
    ...
    def test_bypass_flag_forces_raw_urls_even_when_hosted(self):
        ...
        hosting entirely — never serves our JPEG, even for a fully-ready photo."""
        listing = VehicleListing.objects.create(
    ...
    def test_ready_gumtree_photo_uses_our_hosted_jpeg(self):
        # A fully-hosted photo exists on the listing, plus a second photo that
        # failed hosting — with the temporary bypass off, Gumtree behaves like
        # custom-domain: serve the hosted JPEG where ready, raw URL otherwise.
        listing = VehicleListing.objects.create(
