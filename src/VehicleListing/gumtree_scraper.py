...
def get_gumtree_listings(profile_url, user):
    """Get all listings for a given seller ID."""
    if not API_KEY:
        logging.error("ZENROWS_API_KEY is not configured in the environment variables")
    ...
    try:
        ...
        profile_listings = []
        initial_url = f"{base_url}?page=0&size=5"
        ...
        for current_list in listings:
            ...
            if result and not already_exists:
                ...
                listing_dict = {
                    ...
                    'list_id': vehicle_listing.list_id,
                    'year': vehicle_listing.year,
                    'body_type': vehicle_listing.body_type,
                    'fuel_type': vehicle_listing.fuel_type,
                    'color': vehicle_listing.color,
                    'variant': vehicle_listing.variant,
                    ...
                    'gumtree_profile_id': vehicle_listing.gumtree_profile_id
                }
                profile_listings.append(listing_dict)

        return True, "Extracted all listings successfully", profile_listings
