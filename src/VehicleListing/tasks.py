...
from VehicleListing.models import VehicleListing, GumtreeProfileListing, FacebookProfileListing, RelistingFacebooklisting, Invoice
...
#         logger.info(f'update the failed relisting{listing.year} {listing.make} {listing.model} status to pending to relist again through pending listing process')
#         listing.listed_on=None
#         listing.status='pending'
#         listing.retry_count=0
#         listing.is_relist=False
#         listing.has_images=False
#         listing.save()
#         logger.info(f"delete failed relisting {relisting.listing.list_id} which later recreate through pending listing process")
#         relisting.delete()
#
...
@shared_task(base=CustomExceptionHandler, queue='scheduling_queue')
def check_trial_expiry_task():
    """
    Daily task that:
    1. Sends warning emails at 14 days and 4 days before trial_end_date.
    2. On the expiry date: sets account_status = 'trial_expired' and sends expiry email.
    Runs once per day via Celery Beat.
    """
    from VehicleListing.utils import send_trial_expiry_notification

    logger.info("Starting daily trial expiry check")
    now = timezone.now()
    today = now.date()

    # --- 14-day warning ---
    target_14 = today + timedelta(days=14)
    users_14 = User.objects.filter(
        account_status='trial',
        trial_end_date__date=target_14,
    )
    for user in users_14:
        logger.info(f"Sending 14-day trial warning to {user.email}")
        send_trial_expiry_notification(user, days_remaining=14)

    # --- 4-day warning ---
    target_4 = today + timedelta(days=4)
    users_4 = User.objects.filter(
        account_status='trial',
        trial_end_date__date=target_4,
    )
    for user in users_4:
        logger.info(f"Sending 4-day trial warning to {user.email}")
        send_trial_expiry_notification(user, days_remaining=4)

    # --- Expiry: trial_end_date is today or in the past ---
    expired_users = User.objects.filter(
        account_status='trial',
        trial_end_date__date__lte=today,
    )
    for user in expired_users:
        logger.info(f"Trial expired for {user.email} — updating status and sending expiry email")
        user.account_status = 'trial_expired'
        user.trial_used = True  # Ensure trial cannot be reclaimed even for legacy accounts
        user.save(update_fields=['account_status', 'trial_used'])
        send_trial_expiry_notification(user, days_remaining=0)

    logger.info(
        f"Trial expiry check complete. "
        f"14d warnings: {users_14.count()}, "
