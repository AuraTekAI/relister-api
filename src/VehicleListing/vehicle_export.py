...
Branch note: this branch has no lifecycle_status/first_listed_at/delisted_at/
days_to_sell columns — sale state lives on `status` (+ sold_at/listed_on), so
those four export columns are DERIVED here by _lifecycle_status() and
_days_to_sell(). The exported shape is unchanged, which is what the Dealer Info
frontend consumes.
...
def _lifecycle_status(listing):
    """The lifecycle_status column this branch doesn't store, derived from
    `status`: 'sold' once mark_listing_sold() has run, 'withdrawn' for the
    delete outcomes, 'active' for everything still on the market."""
    if listing.status == VehicleListing.STATUS_SOLD:
        return 'sold'
    if listing.status in VehicleListing.STATUS_WITHDRAWN:
        return 'withdrawn'
    return 'active'


def _days_to_sell(listing):
    """Whole days between going live and selling, for sold listings only.
    None whenever either end of the interval is unknown, or the pair is
    inconsistent (sold before listed) — an undefined duration must not be
    reported as 0 days, which would drag the percentiles down."""
    if listing.status != VehicleListing.STATUS_SOLD:
        return None
    if not listing.listed_on or not listing.sold_at:
        return None
    days = (listing.sold_at - listing.listed_on).days
...
def _row_for_listing(listing, stats):
    ...
    return {
        ...
        'lifecycle_status': _lifecycle_status(listing),
        'listed_on': listing.listed_on.isoformat() if listing.listed_on else None,
        # first_listed_at/delisted_at aren't stored on this branch; listed_on
        # and sold_at are their equivalents.
        'first_listed_at': listing.listed_on.isoformat() if listing.listed_on else None,
        'delisted_at': listing.sold_at.isoformat() if listing.sold_at else None,
        'days_to_sell': _days_to_sell(listing),
...
def export_vehicle_data(request):
    ...
    sold_days = [
        days
        for days in (_days_to_sell(listing) for listing in listings)
        if days is not None
    ]
