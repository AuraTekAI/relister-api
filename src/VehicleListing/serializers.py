...
class VehicleListingSerializer(serializers.ModelSerializer):
    ...
    # Vehicle model fields (refactored database structure)
    ...
    def get_images(self, obj):
        # Use S3-hosted images with master branch S3 pipeline logic
        # Custom-domain sites typically don't return CORS headers, so the
    ...
    def get_location(self, obj):
        # Custom-domain specific logic: use dealer location as fallback
        if obj.location:
            return obj.location
        ...
        full_state = state_full_names.get(state, state)
...
class ProductListSerializer(serializers.ModelSerializer):
    ...
    def get_image(self, obj):
        # Use S3-hosted images when available (master branch pipeline)
...
class ProductDetailSerializer(serializers.ModelSerializer):
    ...
    def get_images(self, obj):
        # Use S3-hosted images for better performance (master branch pipeline)
